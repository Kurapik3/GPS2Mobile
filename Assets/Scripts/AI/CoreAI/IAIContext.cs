...
public interface IAIContext
{
    ...
    //==================== Enemy(/Player) Info ====================
    bool IsEnemyNearby(Vector3 position, int range); //Check if any enemy is within range
    Vector3 GetNearestEnemy(Vector3 fromPosition); //Get the nearest enemy position from a given location
    List<int> GetEnemiesInRange(Vector3 position, int range); //Get all enemy IDs within attack range
