...
public class CombatAI : MonoBehaviour
{
    ...
    {
        EventBus.Subscribe<EnemyAIEvents.ExecuteCombatPhaseEvent>(OnCombatPhase);
    }

    private void OnDisable()
    {
        EventBus.Unsubscribe<EnemyAIEvents.ExecuteCombatPhaseEvent>(OnCombatPhase);
    }

    private void OnCombatPhase(EnemyAIEvents.ExecuteCombatPhaseEvent evt)
    {
    ...
    private IEnumerator RunCombat()
    {
        ...
        foreach (var id in unitIds)
        {
            ...
            //Move toward nearest player base
            TryMoveIdle(id);
            yield return new WaitForSeconds(stepDelay / AIController.AISpeedMultiplier);
        }
    }
    ...
    private List<int> GetEnemiesInRange(Vector2Int from, int range)
    {
        ...
        //var playerUnits = FindFirstObjectByType<UnitManager>(); // assume exists; fallback to tags
        //if (playerUnits != null)
        //{
        //    foreach (var id in playerUnits.GetAllPlayerUnitIds()) // implement in your PlayerUnitManager
        //    {
